...
uint64_t biosyn_highlight_line(biosyn_format_t fmt, const char *line, uint64_t len, uint64_t zero_based_line_no, biosyn_span_t *out, uint64_t out_cap) {
    writer_t w;
    uint32_t phase = (fmt == BIOSYN_FORMAT_FASTQ) ? (uint32_t)(zero_based_line_no & 3u) : 0u;
    ...
    w.count = 0;
    w.has_last = 0;
    dispatch(&w, fmt, line, (size_t)len, phase, NULL);
